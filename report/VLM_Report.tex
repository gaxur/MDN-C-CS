...
\textbf{Key Insight}: Prompt engineering provides substantial improvements for weaker models (LLaVA-v1.5: +10 pp accuracy) but can introduce unnecessary conservatism in stronger models (Qwen3-VL: -1 pp). Architectural improvements remain the most effective hallucination mitigation strategy.
...
Our systematic evaluation with real-world execution demonstrates that Vision-Language Models exhibit substantial performance differences in hallucination susceptibility. LLaVA-v1.5 achieves 53\% accuracy with a 47\% hallucination rate, while Qwen3-VL achieves 92\% accuracy with only 8\% hallucination—a 39-percentage-point gap reflecting fundamental architectural improvements.

Critically, spatial reasoning remains weak across both models, with LLaVA-v1.5 failing catastrophically (12\% accuracy) and even Qwen3-VL achieving only 80\%. This indicates that geometric reasoning remains a fundamental challenge for current \VLMs.

Prompt engineering provides meaningful improvements for weaker models: LLaVA-v1.5's strict prompting improves accuracy by 10 percentage points, with counting accuracy improving dramatically from 56\% to 88\%. However, strict prompting does not improve stronger models and may introduce unnecessary conservatism (Qwen3-VL: 92\% → 91\%).

For practitioners: (1) Architectural improvements (Qwen3-VL over LLaVA-v1.5) provide the largest hallucination reductions, (2) Prompt engineering provides meaningful but limited improvements for weaker models, and (3) No single mitigation strategy suffices for safety-critical spatial reasoning tasks—human verification remains essential.
